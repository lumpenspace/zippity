import os
from zippity.fs_utils import get_gitignore_matcher, collect_files, find_gitignore, collect_non_ignored_files
import pytest
...
def with_gitignore(test_files_directory):
    return os.path.join(test_files_directory, 'fs_test_files', 'directory_with_gitignore')
...
def test_collect_non_ignored_files_valid(without_gitignore):
    ...
    files = list(collect_non_ignored_files(without_gitignore, lambda x: "text/x-python"))
    assert len(files) > 0
